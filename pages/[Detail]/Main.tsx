...
function Main({ifreamload , data} : any) {
  return (
    ...
    <div className={`w-full h-auto py-[50px]   `}>
      ...
      <div className={`relative overflow-hidden w-full ${ifreamload ? "" : "hidden"}   frc justify-center pt-[56.25%] m-auto rounded-0 600:rounded-[8px]`}>
                 {/* @ts-ignore */}
      ...
      <div className='w-full h-auto frc justify-center mt-[40px]'>
          <a href={data?.LIVE_MODE} id='monospace' className={`px-[30px] ${data?.LIVE_MODE !== "" ? "" : "hidden"} shadow-lg  py-[10px]  rounded-[5px] text-white bg-teal-400 text-[20px] hover:scale-110 transition-all duration-100 cursor-pointer`}>
             Live Mode
          </a>
          <div id='monospace' className={`px-[30px] ${data?.LIVE_MODE !== "" ? " hidden" : ""} py-[10px] frc rounded-[5px] text-rose-500 bg-gray-700 opacity-[0.5] text-[20px] hover:scale-110 transition-all duration-100 cursor-pointer`}>
